...
do
	...
	datafile=`grep '^#' "$f" | cut -c 1 --complement`
	cat "$f" | grep '^<' | cut -c 1 --complement >"$tmp_entrada_test"
	cat "$f" | grep '^>' | cut -c 1 --complement >"$tmp_salida_test"

	if [ $CORRER_VALGRIND == 1 ]; then
		valgrind --leak-check=full --show-reachable=yes -q --log-file="$tmp_valgrind" "$PROGRAMA" "$datafile" <"$tmp_entrada_test" >"$tmp_salida" 2>"$tmp_stderr"
	else
		"$PROGRAMA" "$ROOT/$datafile" <"$tmp_entrada_test" >"$tmp_salida" 2>"$tmp_stderr"
	fi
